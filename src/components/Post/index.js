...
const Post = ({ post, refreshPosts }) =>
{
    ...
    useEffect(() =>
    {
        ...
        // Kiểm tra like từ backend hoặc localStorage
        let hasLiked = Array.isArray(post.likes) ? post.likes.some(like => like.liker_id === userId) : false;
        if (!hasLiked)
        {
            // Kiểm tra localStorage nếu backend không trả về like của user
    ...
    const handleLikeUnlike = async () =>
    {
        ...
        try
        {
            ...
            // Fallback nếu không có username/avatar
            if (!userName || userName === "null" || userName === "undefined") userName = "You";
            if (!userAvatar || userAvatar === "null" || userAvatar === "undefined") userAvatar = "/default_avatar.png";
            ...
            {
                // Gọi API và lấy object like trả về
                const response = await likePost(post.id);
                let likeObj = response && response.data ? response.data : null;
                if (!likeObj)
                {
                    // fallback nếu backend không trả về đúng
                    likeObj = {
                        id: Date.now(),
                        liker_id: userId,
                        liker_name: userName,
                ...
                {
                    // Nếu đã có like của user này (do backend trả về), update lại name/avatar nếu cần
    ...
    const handleCommentSubmit = async (e) =>
    {
        ...
        try
        {
            ...
            {
                // Thay vì xóa toàn bộ comment local, chỉ update comment pending thành comment thật
                setPendingComments(pending => pending.filter(c => c.id !== tempId));
                setLocalComments(comments =>
                    comments.map(c =>
                        c.id === tempId
                            ? { ...response.data } // cập nhật thành comment thật từ backend
                            : c
                    )
                );
            }
