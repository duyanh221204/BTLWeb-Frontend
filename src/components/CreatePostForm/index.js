...
const CreatePostForm = ({ onPostCreated, mode = "create", initialContent = "", initialImage = "", initialHashtags = [], postId }) =>
{
    ...
    useEffect(() =>
    {
        ...
        // Đảm bảo chỉ gọi khi showHashtagModal true
    ...
    // Chỉ reset state khi mode hoặc postId thay đổi (tức là chuyển sang sửa post khác hoặc tạo post mới)
    useEffect(() =>
    {
    ...
    // SỬA TRIỆT ĐỂ LỖI LẶP VÔ HẠN: KHÔNG RESET STATE KHI ĐANG Ở TRANG CREATE POST
    useEffect(() =>
    {
        // Reset state chỉ khi RỜI KHỎI trang này và KHÔNG reset khi đang ở /create-post hoặc /edit-post
    ...
    const handleAddHashtag = (tag) =>
    {
        ...
        if (!norm || hashtags.includes(norm)) return;
